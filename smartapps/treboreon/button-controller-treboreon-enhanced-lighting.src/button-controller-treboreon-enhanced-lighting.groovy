...
definition(
    name: "Button Controller - Treboreon Enhanced Lighting",
    namespace: "treboreon",
    author: "Treboreon - Modified from Eric Maycock (erocm123)",
                ...
                for (int i = 1; i <= (numberOfButtons as Integer); i++){
                   ...
                   if (settings["lights_${i}"] != null) { 
                      settings["lights_${i}"].each {
                      	 def inDescription = getInDescription(i, it)
                         configDescription += "${it.displayName} - $inDescription, "
                      }
...
def configureLight(params) {
    ...
    dynamicPage(name: "configureLight", uninstall: false, install: false) {
        section ("$params.name") {
            def switchType_Dimmer = false
            def switchType_Color = false

            switchType_Dimmer = ("$params.capabilities".indexOf('Switch Level') >= 0)
            switchType_Color = ("$params.capabilities".indexOf('Color Control') >= 0)

			input "lights_${params.buttonNumber}_${params.lightId}_power", "bool", title: "Turn the light on or off", submitOnChange: false
	
    		if (switchType_Dimmer)
            {
                ...
                input "lights_${params.buttonNumber}_${params.lightId}_dim", "bool", title: "Dim (off for Brighten)", submitOnChange: false
                input "lights_${params.buttonNumber}_${params.lightId}_lightLevelRelative", "enum", title: "Change Level?", required: false, options: [[10:"10%"],[20:"20%"],[25:"25%"],[50:"50%"]]
			}
...
def getInDescription(buttonNumber, light)
{
               def inDescription = "Click to Configure"
                ...
                if (settings["lights_${buttonNumber}_${light.id}_lightLevelRelative"] != null)
                {
                	if (settings["lights_${buttonNumber}_${light.id}_dim"] != null && settings["lights_${buttonNumber}_${light.id}_dim"])
                		inDescription = "$inDescription, Dim by: ${settings["lights_${buttonNumber}_${light.id}_lightLevelRelative"]}"
                    else
                    	inDescription = "$inDescription, Brighten by: ${settings["lights_${buttonNumber}_${light.id}_lightLevelRelative"]}"
                }
	...
	return {
        ...
        def map = [:]
        	lightsConfigured.each {light ->
            	def inDescription = getInDescription(buttonNumber, light)
            	section ("$light"){
...
def setLight(light, buttonNumber, toggle, sequence = null) {
    ...
    switchType_Dimmer = ("$light.capabilities".indexOf('Switch Level') >= 0)
    switchType_Color = ("$light.capabilities".indexOf('Color Control') >= 0)

	log.trace "setLight $buttonNumber - switchType_Dimmer : $switchType_Dimmer switchType_Color : $switchType_Color"
    ...
    if (settings["lights_${buttonNumber}_${light.id}_lightLevelRelative"] != null)
    	{ levelRelative = settings["lights_${buttonNumber}_${light.id}_lightLevelRelative"] }
    if (settings["lights_${buttonNumber}_${light.id}_dim"] != null)
    	{ dim = settings["lights_${buttonNumber}_${light.id}_dim"] }
    else { dim = false }

	if (settings["lights_${buttonNumber}_${light.id}_color"] != null)
    	{ color = settings["lights_${buttonNumber}_${light.id}_color"] }
    
//    log.trace "setLight $buttonNumber - power : $power"
//    log.trace "setLight $buttonNumber - level : $level"
//    log.trace "setLight $buttonNumber - levelRelative : $levelRelative"
//    log.trace "setLight $buttonNumber - dim : $dim"
//    log.trace "setLight $buttonNumber - color : $color"

    if (toggle) power = false
    
    log.trace "setLight $buttonNumber - power : $power (after toggle : $toggle)"

    if (levelRelative && switchType_Dimmer)
    {        
        log.trace "setLight $buttonNumber - levelRelative : $levelRelative dim : $dim"
        def currentLevel 
        if (light.currentValue("switch") == "on") {
                  currentLevel = light.currentValue("level")
        }
        else
        {
        	log.trace "setLight - Light currently off"
        	currentLevel = 0
        }
         
        def newLevel = 0
        if (dim)
        	newLevel = currentLevel - (levelRelative as Integer)
        else
        	newLevel = currentLevel + (levelRelative as Integer)
        if (newLevel > 100) newLevel = 100
        
        log.trace "setLight $buttonNumber going from $currentLevel to $newLevel"
        if (newLevel > 0)
        	light.setLevel(newLevel as Integer)
        else
        	light.off()
    }
	else if (power == true && sequence != "last") {
		light.on()
	}
    
    if (level && switchType_Dimmer)
    {
    	log.trace "setLight $buttonNumber - Setting Level : $level"
        light.setLevel(level as Integer)
    }

	if (color && switchType_Color)
    {
        def hueColor = 0
        def saturation = 100
...
//  delayBetween was throwing an exception all the time like it was not defined in the language.
//            try{
...
//            }
    ...
    if(power == false && sequence != "first") {
    	light.off()
    }
    
    def switchType = "Remove"
    switch(switchType) {
        case ~/.*Switch.*/:
      	//  light.on()
        break
